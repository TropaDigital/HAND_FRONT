...
class Gerenciador_LoginController extends My_Controller 
{
	...
	protected function login()
    {
    	...
    	$nivel = UsuariosZigzag::nivelGerenciado;
    	
    	$this->auth = Zend_Auth::getInstance();
    	
    	try {
    		
    		$authAd = new Zend_Auth_Adapter_DbTable($this->db,$this->config->tb->login_gerenciador,'login','senha');
    	...
    	if ( $request->getPost('login') && $request->getPost('senha') ){
    		...
    		if ( !empty($login) && !empty($senha) ){
    			...
    			if( $result->isValid() && $authAd->getResultRowObject()->login == $this->view->GerenciadorCustom->login && $authAd->getResultRowObject()->ativo == 1 && $authAd->getResultRowObject()->nivel == $nivel  ){
