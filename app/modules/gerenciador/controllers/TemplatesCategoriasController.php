...
class Gerenciador_TemplatesCategoriasController extends My_Controller
{
	...
	public function ini()
	{
		...
		$this->data = new Model_Data(new categorias_gerenciador());
		$this->data->_required(array('id_categoria', 'categoria', 'id_gerenciador', 'imagem', 'descricao', 'status'));
    ...
    public function indexAction ()
    {
    	...
    	$sql->from(array('CAT'=>$this->config->tb->categorias_gerenciador),array('*'));

    	$sql->where('id_gerenciador = '.$this->me->id_usuario);
    ...
    public function actionAction ()
    {
    	...
    	$post['id_gerenciador'] = $this->me->id_usuario;
    	$post['status'] = 1;
    ...
    public function excluirAction ()
    {
    	...
    	$this->data->_table()->getAdapter()->query('DELETE FROM zz_categorias_gerenciador WHERE id_categoria IN ('.implode(',',$post[id]).')');
